...
# conversion of data to aif360 format
...
#     y = encoded_df[target]
#     filtered_col_name = f'filtered_{col_name}'
#     test_df = overall_df[overall_df['dataset']== 'test'] # Run predict on test,
#     train_df = overall_df[overall_df['dataset']== 'train']
...
def get_fairness_metrics_bc(original_output,col_name):
    ...
    unpriviliged_outcomes = female_df[(female_df['target_predicted'] == 1)&(female_df['actual'] == 1)].shape[0]
    unpriviliged = unpriviliged_outcomes/num_of_unpriviliged
    unpriviliged

    priviliged_outcomes = male_df[(male_df['target_predicted'] == 1)&(male_df['actual'] == 1)].shape[0]
    priviliged = priviliged_outcomes/num_of_priviliged
    priviliged
    true_positive = abs(unpriviliged - priviliged)
    ...
    unpriviliged_outcomes = female_df[(female_df['target_predicted'] == 1)&(female_df['actual'] == 0)].shape[0]
    unpriviliged = unpriviliged_outcomes/num_of_unpriviliged
    unpriviliged

    priviliged_outcomes = male_df[(male_df['target_predicted'] == 1)&(male_df['actual'] == 0)].shape[0]
    priviliged = priviliged_outcomes/num_of_priviliged
    priviliged
    
    false_positive = abs(unpriviliged - priviliged)
    # average
    average_odds_difference = (true_positive + false_positive)/2
    
    #positive predictive value
    ppv = precision_score(original_output['actual'],original_output['target_predicted'])
    
    #negative predictive value
    npv = 1 - original_output[original_output['target_predicted'] == 0]['actual'].mean()
    
    
    #accuracy
    accuracy = (original_output['target_predicted']== original_output['actual']).mean()
    
    return ([disparate_impact,statistical_parity_difference,equal_opportunity_difference,average_odds_difference,ppv,npv,
             accuracy])
...
def fair_model_lfr(data, subset_cols, target, p_att):
    ...
    #print(dataset_orig_train.convert_to_dataframe()[0][target].unique())
    TR = TR.fit(dataset_orig_train, maxiter=5000, maxfun=5000)
    dataset_transf_train = TR.transform(dataset_orig_train)
    dataset_transf_test = TR.transform(dataset_orig_test)
    #print(dataset_transf_train.convert_to_dataframe()[0][target].unique())
    ...
    #print(dataset_transf_train[target].unique())

    classifier = LogisticRegression()
    #print(x_train.columns)
    classifier.fit(dataset_transf_train.drop(target,axis=1), dataset_transf_train[target]) # dropping here
    # We now need to add this array into x_test as a column for when we calculate the fairness metrics.
    y_pred = classifier.predict(dataset_transf_test.drop(target,axis =1))
    x_test = dataset_transf_test
    x_test['target_predicted'] = y_pred
    original_output = x_test
    original_output['actual'] = original_output[target].values
    original_output.to_csv('original_output.csv')
    return original_output

def fair_classifier_lfr(features_encoded_data,target,p_att_col,model_cols, filtered_col, etas):
    
    #print(model_cols)
    try:
        original_output = fair_model_lfr(features_encoded_data, model_cols, target, filtered_col)
        base_classifier_fairness_metrics = get_fairness_metrics_bc(original_output,p_att_col)
        return pd.DataFrame([['lfr_model']+base_classifier_fairness_metrics],columns = ['model_type',\
                                                                                         'disparate_impact',\
        ...
        return pd.DataFrame([['lfr_model']+['model_failed']*7],columns = ['model_type',\
                                                                                         'disparate_impact',\
                                                                                         'statistical_parity_difference',\
                                                                                         'equal_opportunity_difference','average_abs_odds_difference',\
                                                                                         'positive_predictive_value','negative_predictive_value','accuracy'])
...
def get_fairness_metrics_fc(dataset_orig_train,dataset_orig_test,p_att,privileged_groups,unprivileged_groups,etas):
    ...
    for eta in etas:
        ...
        outputs.append([eta,metric.disparate_impact(),metric.statistical_parity_difference(),\
                        metric.equal_opportunity_difference(),metric.average_abs_odds_difference(),metric.positive_predictive_value(),
                        metric.negative_predictive_value(),\
                        accuracy_score(pred.labels, dataset_orig_test.labels)])
    return outputs

def get_fairness_metrics_adversarial(dataset_orig_train,dataset_orig_test,p_att,privileged_groups,unprivileged_groups,etas):
    ...
    unprivileged_groups = unprivileged_groups,scope_name=f'debiased_classifier_{int(random.random()*1000000)}',debias=True,sess=sess)        
    model = debiased_model.fit(dataset_orig_train)
    pred = model.predict(dataset_orig_test)
    ...
    outputs.append([metric.disparate_impact(),metric.statistical_parity_difference(),\
    metric.equal_opportunity_difference(),metric.average_abs_odds_difference(),metric.positive_predictive_value(),
                        metric.negative_predictive_value(),\
    accuracy_score(pred.labels, dataset_orig_test.labels)])
    return outputs
...
#merging dataframes
def generate_xy_greater_xa(data1,data2,greater=False):
    '''
    ...
    for a_v, df_a in merged_xiY_xiA.groupby('A'):
        dictonary_xy_greaterthan_xa[a_v] = df_a[df_a['X_sub_A = True']== greater]['X'].values #XY>XA but with true it would give XA>XY

    return dictonary_xy_greaterthan_xa


# Criteria 1
def xy_greater_xa(data,features_cat,features_num,protected_attributes, target,feat_dictionary, greater=False,k = 5):
    '''
    ...
    dict_subsets_xy_greaterthan_xa = generate_xy_greater_xa(mi_Xi_Y,mi_Xi_A,greater) 
    print (dict_subsets_xy_greaterthan_xa)
    return features_encoded_data, dict_subsets_xy_greaterthan_xa

# no feature selection
def no_fselection(data,features_cat,features_num,protected_attributes, target,feat_dictionary, greater = False,k=5):
...
def sequential_feature_selection_forward(data,features_cat,features_num,protected_attributes, target,feat_dictionary, greater = False,k=5):  
    #return  
    '''
...
# Backward sfs
def sequential_feature_selection_backward(data,features_cat,features_num,protected_attributes, target,feat_dictionary, greater=False,k=5):
    '''
...
def sequential_fs(data,features_cat,features_num,protected_attributes, target,feat_dictionary,greater=False,k=5):
    features_encoded_data,enc = categorical_feature_encoder(data.copy(),features_cat + protected_attributes)
    dictionary_features = {}
    for att in protected_attributes:
...
def maximally_predictive_minimally_demographic(data,features_cat,features_num,protected_attributes, target,feat_dictionary,greater=False,k=5): # add feat_dictionary
    '''
...
# Criteria 4 
# calculated I(X,A| Y=y) for y=0 and y =1
# calculated $I(X_i,A |Y =0) - I(X_i,A|Y=1)
# arranged in descending order and chose top K
    
def sufficiency_criteria(data,features_cat,features_num,protected_attributes, target,feat_dictionary,greater=False,k=5): 
    '''
    takes data and returns encoded df and a dictionary with keys as protected att and values as features 
    (features = all features where XY>XA for a particular protected att) 
    ...
    subsets = []
    feature_subset = {}
    for y in features_encoded_data[target].unique():
        temp = features_encoded_data[features_encoded_data[target]==y]
        temp_xa = calculate_miscore_xa(temp,protected_attributes)
        temp_xa = temp_xa.rename(columns = {'I(Xi,A)':f'I(Xi,A)_y={int(y)}'})
        temp_xa = temp_xa.set_index(['X','A'])
        subsets.append(temp_xa.copy())
    concat = pd.concat(subsets, axis =1)
    #print(concat.columns)
    if features_encoded_data[target].nunique() == 2:
        concat['diff'] = abs(concat['I(Xi,A)_y=0']- concat['I(Xi,A)_y=1'])
        
    else:
        concat['diff'] = concat.var(axis=1)
        
    concat['diff'].sort_values(ascending= True)
    concat = concat.reset_index()
    #print(k)
    for a, temp in concat.groupby('A'):
        feature_subset[a] = list(temp['X'].iloc[:k].values)
    return features_encoded_data, feature_subset
...
def fair_classifier_adversarial(features_encoded_data,target,p_att_col,model_cols, filtered_col, etas):
    '''
    takes encoded df, runs prejudice remover on the data to return a df consiting of fairness metrics and model type column.
    ...
    #print(model_cols)
    dataset_orig_train,dataset_orig_test = fair_model(features_encoded_data, model_cols, target, filtered_col)
    fair_classifier_fairness_metrics = get_fairness_metrics_adversarial(dataset_orig_train,dataset_orig_test, \
                                                               filtered_col,privileged_groups = [{filtered_col:1}] ,\
                                                               unprivileged_groups = [{filtered_col:0}],etas=etas)
    result = pd.DataFrame(fair_classifier_fairness_metrics,columns = ['disparate_impact',\
                                                                      'statistical_parity_difference',\
                                                                      'equal_opportunity_difference','average_abs_odds_difference',\
                                                                      'positive_predictive_value', 'negative_predictive_value', 'accuracy'])
    result.insert(0,'model_type','fair_model_adversarial')
    return result
...
def main_exp_bf_func(data,features_cat,features_num,protected_attributes, target,p_att_description, etas, feat_dictionary,\
                     technique = xy_greater_xa, model = fair_classifier,greater=False, k =5):
    ...
    features_encoded_data, dict_subsets_xy_greaterthan_xa = technique(data_df,features_cat,features_num,protected_attributes, target,feat_dictionary,greater, k)
